        ...
        if (hand == Hands.left)
        {
            if(Input.GetKeyDown(KeyCode.JoystickButton2)) // X
            {
                if (leftEnter == true && leftObj != null)
                {
                    ...
                    {
                        SnapToggle(leftObj, ref leftCarrying, transform.parent);
                    }
                }
            }
            if(Input.GetKeyDown(KeyCode.JoystickButton3)) // Y
            {
                if (leftEnter == true && leftObj != null)
                {
                    if (leftObjTag == "Snap Toggle")
                    {
                        Holster(leftObj, ref leftCarrying, transform.parent, leftHolster);
                    }
                }
            }
            ...
            if (leftEnter == true && leftObj != null && leftObjTag == "Free Grab")
            {
                ...
                {
                    Grab(leftObj, transform.parent, ref leftCarrying);
                }
                else if(left_CanGrab == false && leftCarrying == true)
                {
                    Release(leftObj, transform.parent, GetComponent<Rigidbody>(),  ref leftCarrying);
                }
            }
        ...
        else if(hand == Hands.right)
        {
            if(Input.GetKeyDown(KeyCode.JoystickButton0)) // A
            {
                if (rightEnter == true && rightObj != null)
                {
                    ...
                    {
                        SnapToggle(rightObj, ref rightCarrying, transform.parent);
                    }
                }
            }
            if(Input.GetKeyDown(KeyCode.JoystickButton1)) // B
            {
                if (rightEnter == true && rightObj != null)
                {
                    if (rightObjTag == "Snap Toggle")
                    {
                        Holster(rightObj, ref rightCarrying, transform.parent, rightHolster);
                    }
                }
            }
            ...
            if (rightEnter == true && rightObj != null && rightObjTag == "Free Grab")
            {
                ...
                {
                    Grab(rightObj, transform.parent, ref rightCarrying);
                }
                else if(right_CanGrab == false && rightCarrying == true)
                {
                    Release(rightObj, transform.parent, GetComponent<Rigidbody>(),  ref rightCarrying);
                }
            }
        }
        ...
        if(hand == Hands.left)
        {
            foreach(XRNodeState nodeState in nodeStates)
            {
                if(nodeState.nodeType == XRNode.LeftHand)
                {
                    ...
                    _obj.GetComponent<Rigidbody>().angularVelocity = angVel;
